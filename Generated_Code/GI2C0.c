...
#define GI2C0_TIMEOUT_US  ((uint32_t)1000)  /* number of microseconds as specified in properties */
#define GI2C0_TIMEOUT_NOF_TICKS(factor) ((GI2C0_TIMEOUT_US*(factor))/1000/TMOUT1_TICK_PERIOD_MS)
#define GI2C0_TIMEOUT_TICKS(factor) (GI2C0_TIMEOUT_NOF_TICKS(factor)>0?GI2C0_TIMEOUT_NOF_TICKS(factor):1)  /* at least one tick */
...
byte GI2C0_ReadBlock(void* data, word dataSize, GI2C0_EnumSendFlags flags)
{
  ...
  for(;;) { /* breaks */
    ...
    timeout = TMOUT1_GetCounter(GI2C0_TIMEOUT_TICKS(dataSize)); /* set up timeout counter */
...
byte GI2C0_ReadAddress(byte i2cAddr, byte *memAddr, byte memAddrSize, byte *data, word dataSize)
{
  ...
  for(;;) { /* breaks */
    ...
    timeout = TMOUT1_GetCounter(GI2C0_TIMEOUT_TICKS(memAddrSize)); /* set up timeout counter */
    if (timeout==TMOUT1_OUT_OF_HANDLE) {
      res = ERR_QFULL;
      break; /* break for(;;) */
    }
...
byte GI2C0_WriteAddress(byte i2cAddr, byte *memAddr, byte memAddrSize, byte *data, word dataSize)
{
  ...
  for(;;) { /* breaks */
    ...
    timeout = TMOUT1_GetCounter(GI2C0_TIMEOUT_TICKS(i)); /* set up timeout counter */
    if (timeout==TMOUT1_OUT_OF_HANDLE) {
      res = ERR_QFULL;
      break; /* break for(;;) */
    }
...
byte GI2C0_ScanDevice(byte i2cAddr)
{
  ...
  for(;;) { /* breaks */
    ...
    timeout = TMOUT1_GetCounter(GI2C0_TIMEOUT_TICKS(1)); /* set up timeout counter */
    if (timeout==TMOUT1_OUT_OF_HANDLE) {
      res = ERR_QFULL;
      break; /* break for(;;) */
    }
